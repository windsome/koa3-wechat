...
export default class Media extends Base {
  ...
  async uploadMedia(filepath, type, filename, mime) {
    ...
    var url = 'https://api.weixin.qq.com/cgi-bin/media/upload?access_token=ACCESS_TOKEN&type=TYPE'.replace(
      /TYPE/g,
      type
    );
    var opts = {
  ...
  getMedia(mediaId, timeout = 60000) {
    var url = 'https://api.weixin.qq.com/cgi-bin/media/get?access_token=ACCESS_TOKEN&media_id=MEDIA_ID'.replace(
      /MEDIA_ID/g,
      mediaId
    );
    var opts = {
