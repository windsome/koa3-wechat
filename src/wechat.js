    ...
    return async function(ctx, next) {
      ...
      if (method === 'GET') {
        ...
        if (encrypted) {
          ...
          valid =
            signature === that.cryptor.getSignature(timestamp, nonce, echostr);
        } else {
          // 校验
          valid =
        ...
        var replyMessageXml = that.reply(
          ctx.body,
          formated.ToUserName,
          formated.FromUserName
        );
        ...
        if (!query.encrypt_type || query.encrypt_type === 'raw') {
          ...
          wrap.signature = that.cryptor.getSignature(
            wrap.timestamp,
            wrap.nonce,
            wrap.encrypt
          );
